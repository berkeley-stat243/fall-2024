...
[PDF](./unit9-sim.pdf){.btn .btn-dark}
...
estimate of $\phi$:
$$\hat{\phi}=\frac{1}{m}\sum_{i=1}^{m}h(Y_{i})=\frac{1}{m}\sum_{i=1}^{m}\hat{\beta}_{i}=\hat{E}(\hat{\beta}).$$
...
Note that coverage can be too low for two reasons: (1) the estimated uncertainty
is too low, or (2) the estimator is biased. 
...
:::{.callout-warning "Simulation (Monte Carlo) uncertainty is not statistical uncertainty"}
This is the uncertainty in our simulation-based estimate
...
sequential Monte Carlo (SMC) or Markove chain Monte Carlo (MCMC)), in which case this variance estimator,
...
```{python}
#| fig-alt: Scatterplot of aligned normal and t random variables
...
that the next value is $$x_{k}=f(x_{k-1},\ldots,x_{k-j})\mbox{mod}\,m$$
...
generates the random standard uniform value as $u_k = x_{k}/m$, which by
...
congruential generator: $$x_{k}=(ax_{k-1}+c)\mbox{mod}\,m$$ with integer
$a$, $m$, $c$, and $u_{k}$ values. (Note that in some cases $c=0$, in which case the periodicity can't exceed $m-1$ as the method is then set up so that we never get $x_{k}=0$ as this causes the
algorithm to break.) The seed is
the initial state, $x_{0}$ - i.e., the point in the sequence at which we
...
```{python, fig.height=5}
#| fig-alt: Time series and histograms of pseudo-random numbers
n = 100
a = 171
m = 30269
...
It's the default in R and the old default in numpy (see next section for what I mean by "old default").
...
in the fashion of older versions of numpy actually uses the Mersenne Twister.
...
If we simply start using numpy or scipy to generate random numbers without
choosing the generator using ,
...
some commonalities with rejection sampling. Importance sampling comes up in
a wide variety of contexts, so we'll try to develop some intuition
for how it works and how to reduce variance when using IS.

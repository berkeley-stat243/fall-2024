...
[PDF](./unit2-dataTech.pdf){.btn .btn-info}
...
# 4. Webscraping and working with HTML, XML, JSON, and YAML
...
authentication. That said, it's now fairly old.
...
The main theme of this section is that we can use tools that understand the structure
of formats such as HTML, XML, JSON, and YAML to make it easy to extract the information
we want from a file or webpage.
...
packages for interfacing with Twitter via its API. (Actually, with the change to
X, I don't know if this API is still available.)
...
that represents the character in bytes.
...
![Linked list (courtesy of computersciencewiki.org)](linked-list.png){fig-alt:"Drawing of linked list"}
...
![Tree (courtesy of computersciencewiki.org)](tree.png){fig-alt:"Drawing of tree"}


![Graph (courtesy of computersciencewiki.org)](graph.png){fig-alt:"Drawing of graph"}

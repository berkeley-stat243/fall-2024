...
and `next` seems to do the same thing as `myit.__next__()`? We'll discuss that when we discuss the [#Python object model](#the-python-object-model-and-dunder-methods).
...
class tsSimClass:
    ...
    def __init__(self, times, mean = 0, corParam = 1, seed = 1):
        ## For robustness, need checks that `corParam` is numeric of length 1 and `times` is np array.
        ## Public attributes
        self.n = len(times)
        self.mean = mean

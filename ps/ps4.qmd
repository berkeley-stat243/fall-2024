...
2. Let's work more on how Python stores and copies objects, in particular lists.
   ...
   What is different about how these objects are stored?

3. Suppose I want to compute the trace of a matrix, $A$, $A=XY$, where the trace is $\sum_{i=1}^{n} A_{ii}$. Assume both $X$ and $Y$ are $n \times n$. A naive implementation is `np.sum(np.diag(X@Y))`.
   ...
   b. How could you (much) more eﬀiciently compute the trace in Python using vectorized operations on the matrices. Please provide Python code and do not use `map()` or list comprehension. What is the computational complexity of your solution?
   ...
   e. (Extra credit) Implement your more efficient version in using Jax (see Section 9 of Unit 5) and compare the timing to the numpy implementation, both with and without using `jax.jit()`.
...
likely to be a 2. We could do this using a for loop over the columns of the
